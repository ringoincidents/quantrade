...
### 8-1. 과거 트랙 전수 수집 결과 및 baseline 대비 결론 (2026-08-02)

전수 수집 완료(706건, 판단일 25개, 2026-02-02~07-31). 상세 수치와 룩어헤드 검증은
`Phase2_과거뉴스백테스트_설계.md` §8을 보고, 여기에는 **이 트랙에서 나온 결론만** 남긴다.

**결론: 고확신군조차 단순 baseline보다 낮다.**

뉴스를 전혀 읽지 않고 "항상 다수클래스(해당 기간 실제 수익률의 다수 방향)"만 답하는
무지성 예측기와 비교했다. 비교 표본은 승률 계산에 쓰인 것과 동일하다(중립 제외, 그룹별로
따로 계산).

| window | 고확신군 실제 | baseline | 격차 |
|---|---|---|---|
| D+1 | 46.28% (n=363) | 55.10% | **-8.82p** |
| D+5 | 48.48% (n=363) | 51.52% | **-3.04p** |
| D+20 | 48.62% (n=325) | 54.15% | **-5.53p** |

세 window 전부 baseline 미달이다. 저확신군은 더 나쁘다(각 -26.23p / -4.37p / -14.62p).
고확신·저확신 6개 셀 중 baseline을 넘는 것이 **하나도 없다.**

**이 결과가 §8-0 판정에 갖는 의미 — 판정 자체는 바뀌지 않는다.**
원래도 "이 트랙만으로 §2.1 게이트 통과를 선언하지 않는다"였고 그대로다. 다만 근거의
성격이 달라졌다:

- **기존**: "학습데이터 오염(설계 §4-1) 가능성 때문에 좋은 결과를 신뢰할 수 없다" —
  판단을 *유보*하는 논리.
- **추가**: "애초에 baseline도 못 넘는다" — 직접적인 *부정* 증거.

그리고 이 부정 증거는 **오염 논리와 독립적으로 성립한다.** 오염이 있었다면 오히려
결과가 유리하게 나왔어야 하는데도 baseline 미달이므로, 오염 여부를 확정하지 않고도
"현재 이 판단 방식은 쓸 수 없다"는 결론이 나온다(설계 문서 §8-0 비대칭 해석표의
"불성립" 행에 해당).

**D+1의 "강한신호"(p=0.0343)를 오해하지 말 것.** 고확신군이 좋아서가 아니라 저확신군이
유독 나빠서(baseline 대비 -26.23p) 생긴 격차다. "고확신일 때 잘 맞힌다"가 아니라
"저확신일 때 특히 못 맞힌다"로 읽어야 하며, 고확신 판단을 신뢰 근거로 쓸 수는 없다.

**실시간 트랙은 그대로 계속 돈다.** 판정 권한은 여전히 실시간 트랙에 있고(최초 코호트
D+20 = 2026-08-21), 이 결과 때문에 실시간 트랙을 중단하거나 기준을 바꾸지 않는다.
사전 고정 판정기준(§7의 단측 Fisher's/α구간/최소표본 10)도 무변경이다.
